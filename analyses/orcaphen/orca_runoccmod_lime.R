...
uci<-0.75
prob<-0.5
dat<-read.csv("analyses/output/limedat.csv",header=T)
...
sink("analyses/splineslimekilnmod.txt")
cat("
    model { 
...
if(pod=="J"){y <- dat$Jpres}
if(pod=="K"){y <- dat$Kpres}
if(pod=="L"){y <- dat$Lpres}
if(pod=="SR"){y <- dat$AllSRpres}

# Simulation parameters
#ni=15000; nc=2; nb=0; nt=10
ni=7500; nc=2; nb=2500; nt=1
# List input data
jags.data <- list("nyear","year","nknots","n","X","nc", "nb", "ni", "nt","y")

# Inits function
f.inits <- function(){list(a=rep(0,nyear), b=rep(0,nyear), c=rep(0,nyear))}
...
jags.out<-jags.parallel(jags.data,f.inits,parameters,"analyses/splineslimekilnmod.txt",nc,ni,nb,nt)
