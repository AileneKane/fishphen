...
\par The above occupancy models cannot fully statistically account for changes in effort over time, due to the presence-only nature of the data. We therefore compared phenology estimates from the occupancy models to phenology observed at Lime Kiln state park from 1990 to 2017, which represents a dataset with consistent observer effort from May through September \citep{olson2018}. \\
\underline{Salmon}:
...
\item Expected phenological change due to change in effort alone (simulations)
