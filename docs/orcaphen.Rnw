...
\item Analysis
\begin{enumerate}
\item We used linear regression to identify trends over time in first observation and last observation dates for SRKWs in the Upper Salish Sea and in Puget Sound. 
\item To estimate effects of increased effort (i.e. increased numbers of sightings over time) on trends in phenology, we simulated a dataset of whale presence during a season equivalent to those in our dataset (i.e. October through March, XX days). We used a probability XX for whale presence (the mean in our dataset) and kept it constant over 40 simulated years. We then created ab observation dataset, in which effort (the number of observations), differed. During the low effort time period (years 1-20), the number of observations had a mean of XX and a variance of XX (matching the OrcaMAster database from 1976-1996).  During the high effort rtime period (years 21-40 in our simulated dataset, the number of observations had a mean of XX and a variance of XX (comparable to those in the OrcaMAster database from 1997-2017). We then calculated first- and last- observations dates for each simulated year. We ran these simulatiosn 100 times and calculated the difference between the low effort and high effort time periods (see Supplemental materials, figure).
