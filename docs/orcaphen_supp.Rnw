...
print(salm.table, include.rownames=FALSE, hline.after = c(-1,0,1,17),caption.placement="top",size="\\footnotesize")
@

<<label=limetab,echo=FALSE,results=tex>>=
limetab<-read.csv("../analyses/output/limephen.csv",header=TRUE)
#table of estimates of shifts over time with different threshold probabilitiesand different breakpoints
lime.table<-xtable(limetab,caption = "\\textbf{Estimated shifts with varying breakpoints and threshold probabilities for estimating phenology} i.",
            label="tab:salmon",align=c(
              "|p{0.01\\textwidth}", 
              "|p{0.16\\textwidth}",
               "|p{0.3\\textwidth}",
...
print(lime.table, include.rownames=FALSE, hline.after = c(-1,0,1,17),caption.placement="top",size="\\footnotesize")
@
...
\par Table of trends in different phenophases for salmon
