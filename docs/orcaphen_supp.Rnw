...
pstrendstab<-t(pstrends)
colnames(pstrendstab)<-c("mean","25%", "75%","2.5%","97.5%")
pstrendstab<-pstrendstab[-1,]
phenophase<-c("first","","peak","","last","")
#parameter<-c("$\\alpha_{group}$","$\\beta_{year}$","$\\alpha_{group}$","$\\beta_{year}$","$\\alpha_{group}$","$\\beta_{year}$") 
parameter<-c("intercept","year","intercept","year","intercept","year") 
pstrendstab<-cbind(phenophase,parameter,pstrendstab)

albion<-read.csv("../analyses/output/albionreturntrends_linmodyrs.csv",header=TRUE)
albion<-albion[,4:9]
colnames(albion)<-c("phen","mean","25%", "75%","2.5%","97.5%")
alb<-rbind(albion[1:2,2:6],albion[5:6,2:6],albion[3:4,2:6])

phenophase<-c("first","","peak","","last","")
parameter<-c("intercept","year","intercept","year","intercept","year") 

alb<-cbind(phenophase,parameter,alb)
alltab<-rbind(alb,pstrendstab)
region<-c("Central Salish Sea","","","","","","Puget Sound Proper","","","","","")
alltab<-cbind(region,alltab)
salmtrends.table<-xtable(alltab,caption = "\\textbf{Salmon phenology has shifted} later in part of the Central Salish Sea (based on spring/summer Chinook in the Albion Test Fishery data, from 1995-2017) and earlier in Puget Sound Proper (based on 13 runs across coho, chum, and Chinook adult in Table S1, from 1997-2017).",
...
print(salmtrends.table, include.rownames=FALSE, hline.after = c(-1,0,6,12),caption.placement="top",size="\\footnotesize")#,sanitize.text.function = identity
@

<<label=occmodsums,echo=FALSE,results=tex>>=
modsumfiles<-dir("../analyses/output")[grep("ci0.5occprob_wrecent",dir("../analyses/output"))]
allsums<-c()
...
colnames(allsums)[10:14]<-c("slope.recent","slope.lci.recent","slope.uci.recent","X2.5.recent","X97.5.recent")

modsumtab<-subset(allsums, select=c(pod,Region,Season,phase,slope.mn,slope.lci,slope.uci,X2.5.,X97.5.,slope.recent,slope.lci.recent,slope.uci.recent,X2.5.recent,X97.5.recent))
colnames(modsumtab)<-c("Pod","Region","Season","Phase","mean","25%", "75%","2.5%","97.5%","mean","25%", "75%","2.5%","97.5%")
addtorow <- list()
  addtorow$pos <- list(-1)
  addtorow$command <- c("\\hline & & & & \\multicolumn{5}{c |}{1978-2017 trend} &\\multicolumn{5}{c |}{2002-2017 trend}\\\\\n ")
...
print(modsum.table, include.rownames=FALSE, hline.after = c(0,6,12,18),caption.placement="top",size="\\footnotesize",sanitize.rownames.function=identity,add.to.row=addtorow)
@
