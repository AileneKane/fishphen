...
\par caption for figure of whale daysLong-term trends differ from recent trends for whale sightings (A), whale days (B), and day of year of peak occurrence probabilities in the Central Salish Sea (C) and Puget Sound proper (D). Phenology of peak occurrence probability for J pod is shown here; see Supplemental Materials for time series for K and L pods, as well as time series for other phenological estimates (i.e., first- and last- dates when occurrence probability is greater than 0.5). Gray shading in panels A and B shows the time period during which we focus this paper, to avoid trends driven by strong changes in effort. In panels C and D, shading around line represents 80% credible intervals.
